    ...
    if tool_name == "Read":
        try:
            ...
            if info.get("claimed") and info.get("session_id") != session_id:
                owner = info.get("session_id", "unknown")[:16]
                owner_task = info.get("task") or "unknown task"
                print(
                    f"[FILE-INFO] {os.path.basename(file_path)} is being edited by "
                    f"session {owner} ({owner_task}). Coordinate before modifying."
                )
        except (ImportError, Exception):
            pass
        return

    # On Edit/Write/NotebookEdit: auto-claim
